...
let teamName = fifaData.filter((data) => {
...
if (teamName[0]["Home Team Goals"] > teamName[0]["Away Team Goals"]) {
    console.log(teamName[0]["Home Team Name"])
} else {
    console.log(teamName[0]["Away Team Name"])
};
...
function getFinals(data) {
    ...
    /* code here */
    let getFinals = data.filter((data) => {
...
function getWinners(data, callback) {
    ...
    let finals = callback(data).filter(item => {
...
function getWinnersByYear(cb1, cb2 ) {
    ...
    for(let i = 0; i <cb2.length ; i++){
        winnersbyYear.push(`In ${cb2[i]}, ${cb1[i]} won the world cup!`);
    }
   
    return winnersbyYear;
};

console.log(getWinnersByYear(getWinners(fifaData,getFinals), getYears(fifaData,getFinals)));
